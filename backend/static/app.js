...
function updateRouteLabels(id, entry) {
  ...
  const r = entry.data;
  const points = entry.line.getLatLngs();
  if (!points || points.length < 1) return;

  const routeColor = strToColor(id);
  points.forEach((p, idx) => {
    const mk = L.marker(p, {
      interactive: true,
      icon: L.divIcon({
        className: 'node-hop-label',
        html: String(idx),
        iconSize: [20, 20]
      })
    }).addTo(routeLayer);
...
if (showHopsToggle) {
  const update = () => {
    showHopsToggle.classList.toggle('active', showHops);
    showHopsToggle.textContent = showHops ? 'Hops visible' : 'Show hops';
  };
  update();
  showHopsToggle.addEventListener('click', () => {
    showHops = !showHops;
    localStorage.setItem('meshmapShowHops', String(showHops));
    update();
